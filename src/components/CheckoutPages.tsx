...
import { Plus, Edit, Trash2, Eye, Copy, Settings, QrCode, Share } from 'lucide-react';
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Checkout Pages</h1>
          <p className="text-gray-600">Create and manage your crypto payment pages</p>
        </div>
        <button
      ...
      {checkoutPages.length === 0 ? (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No checkout pages yet</h3>
          <p className="text-gray-500 mb-6">Create your first checkout page to start accepting crypto payments</p>
          <button
            onClick={handleCreatePage}
          ...
          {checkoutPages.map(page => (
            <div key={page.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="h-32 bg-gradient-to-br from-blue-50 to-purple-50 relative overflow-hidden">
                {page.banner ? (
                  <img src={page.banner} alt="Banner" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-blue-100 to-purple-100" />
                )}
                {page.logo && (
                  <div className="absolute top-2 left-2">
                    <img src={page.logo} alt="Logo" className="h-8 w-auto bg-white rounded p-1" />
                  </div>
                )}
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mb-2 line-clamp-2">{page.description}</p>
                    <div className="flex items-center space-x-4 text-sm text-gray-600">
                      <span className="font-medium text-lg text-blue-600">${page.price}</span>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={() => window.open(`/checkout/${page.id}`, '_blank')}
                      className="text-gray-400 hover:text-teal-600 transition-colors"
                      title="Preview page"
                    >
...
function CreatePageModal({ onClose, onViewChange }: { onClose: () => void; onViewChange: (view: string, data?: any) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Create Checkout Page</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              required
            />
            <p className="text-xs text-gray-500 mt-1">Required: Your brand logo will appear on every checkout page</p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Optional: Hero banner image for your checkout page</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Payout Chain *</label>
            <select
              value={formData.payoutChain}
              onChange={(e) => setFormData(prev => ({ ...prev, payoutChain: e.target.value }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
            >
